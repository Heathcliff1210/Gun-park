...
// Self-ping to prevent sleeping (runs every 3 minutes)
// Also includes Telegram webhook pings to maintain bot activity
function selfPing() {
  const urls = [
...
app.listen(PORT, () => {
  ...
  // Start self-pinging every 3 minutes (180,000 ms) to keep bot active
  setInterval(selfPing, 180000);
  
  // Initial ping after 30 seconds
  setTimeout(selfPing, 30000);
  
  // Also add periodic bot health checks
  setInterval(() => {
    // Ping the main bot's health endpoint
    require('http').get('http://localhost:5000/health', (res) => {
      console.log(`✓ Bot health check successful - Status: ${res.statusCode}`);
    }).on('error', (err) => {
      console.log(`⚠ Bot health check failed - ${err.message}`);
    });
