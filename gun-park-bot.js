...
bot.on('text', async (ctx) => {
  ...
  // Handle special commands during test
  if (text === 'stop' || text === 'restart') {
...
async function handleChallengeResponse(ctx, userResponse) {
  ...
  try {
    ...
    // Validate response
    if (userResponse.trim().toLowerCase() === 'stop' || userResponse.trim().toLowerCase() === 'arrêt') {
      userStates.set(telegramId, { step: 'ready' });
...
async function handleTestControl(ctx, command) {
  ...
  const userState = userStates.get(telegramId);
  if (!userState?.chronometer) return;
