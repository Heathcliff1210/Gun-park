    ...
    const response = await fetch(`${COMET_BASE_URL}/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'Génère exactement 20 phrases en français TOTALEMENT ALÉATOIRES et ABSOLUMENT UNIQUES. AUCUNE phrase ne doit ressembler à une autre, même partiellement. Mélange tous les domaines : science, histoire, cuisine, sport, technologie, nature, art, politique, médecine, voyage, architecture, musique, littérature, géographie, astronomie, biologie, chimie, physique, psychologie, sociologie, économie, philosophie, etc. VARIE ÉNORMÉMENT LES LONGUEURS : certaines très courtes (3-6 mots), d\'autres courtes (7-10 mots), quelques moyennes (11-16 mots), et parfois longues (17-25 mots). Tu peux utiliser des dates et des chiffres, mais évite les nombres de plus de 6 chiffres (évite les numéros de téléphone, codes postaux longs, etc.). Les dates comme "1789", "2024", "1969" sont parfaites. Varie les styles : déclaratif, interrogatif, exclamatif, conditionnel. Sois complètement imprévisible et créatif. Une phrase par ligne, pas de numérotation. CHAQUE phrase doit être UNIQUE et différente des autres.'
          },
          {
            role: 'user',
...
// Handle retry challenge actions
bot.action(/^retry_challenge_(\d+)_(\d+)$/, async (ctx) => {
  const match = ctx.match;
  const targetWPM = parseInt(match[1]);
  const sentenceCount = parseInt(match[2]);
  
  const telegramId = ctx.from.id.toString();
  userStates.set(telegramId, { step: 'challenge_wpm_selection', sentenceCount });
  
  await startChallenge(ctx, targetWPM);
});
...
async function startChallenge(ctx, targetWPM) {
  ...
  console.log(`Bot: Starting challenge with ${sentenceCount} sentences at ${targetWPM} WPM for user ${telegramId}`);
  
  try {
    // Try edit first, if fails, send new message
    try {
      await ctx.editMessageText('⚡ PRÉPARATION DU CHALLENGE ⚡\n\n🔄 Génération de phrases aléatoires par IA...\n⏱️ Calcul des temps cibles...\n🎯 Configuration du défi...', 
        Markup.inlineKeyboard([])
      );
    } catch (editError) {
      console.log('Bot: Edit failed, sending new message instead');
      await ctx.reply('⚡ PRÉPARATION DU CHALLENGE ⚡\n\n🔄 Génération de phrases aléatoires par IA...\n⏱️ Calcul des temps cibles...\n🎯 Configuration du défi...');
    }

    // Generate sentences
    console.log('Bot: Generating sentences...');
    const sentences = await generateSentences();
    console.log(`Bot: Generated ${sentences?.length || 0} sentences`);
    
    if (!sentences || sentences.length < sentenceCount) {
      console.error(`Bot: Insufficient sentences: got ${sentences?.length || 0}, needed ${sentenceCount}`);
      throw new Error(`Insufficient sentences generated: got ${sentences?.length || 0}, needed ${sentenceCount}`);
    }
    
    const selectedSentences = sentences.slice(0, sentenceCount);
    console.log('Bot: Selected sentences for challenge:', selectedSentences.length);
    ...
    let errorMessage = '❌ Erreur lors de la préparation du challenge.';
    
    if (error.message.includes('Insufficient sentences')) {
      errorMessage = '❌ Erreur de génération de phrases. Réessaie dans quelques instants.';
    } else if (error.message.includes('CometAPI') || error.message.includes('fetch')) {
      errorMessage = '❌ Problème de connexion à l\'IA. Réessaie.';
    }
    
    await ctx.reply(`${errorMessage}\n\n🔄 Utilise les boutons pour réessayer.`, 
      Markup.inlineKeyboard([
        [Markup.button.callback('🔄 Réessayer', `retry_challenge_${targetWPM}_${sentenceCount}`)],
        [Markup.button.callback('🔙 Retour', 'challenge_mode')]
      ])
    );
  }
...
async function startChallengeSentence(ctx, session, sentence) {
  ...
  try {
    ...
    // Use reply instead of edit to avoid message editing conflicts
    let calculationMessage;
    try {
      calculationMessage = await ctx.editMessageText('🔄 Calcul en cours...\n\nAnalyse de la phrase et calcul du temps cible...', 
        Markup.inlineKeyboard([])
      );
    } catch (editError) {
      console.log('Bot: Edit failed for calculation message, using reply');
      calculationMessage = await ctx.reply('🔄 Calcul en cours...\n\nAnalyse de la phrase et calcul du temps cible...');
    }
    ...
    // Always send a new message for the challenge sentence to avoid edit conflicts
    await ctx.reply(`📝 CHALLENGE - Phrase ${sentenceNumber}/${totalSentences}\n🎯 Objectif : ${session.targetWPM} WPM en max ${targetString}\n\n"${sentence}"\n\n⌨️ Recopie cette phrase exactement :`);
...
async function handleWPMInput(ctx, message) {
  ...
  if (isNaN(wpm) || wpm < 10 || wpm > 500) {
    await ctx.reply('❌ Vitesse invalide. Entre un nombre entre 10 et 500 WPM :');
    return;
  }
