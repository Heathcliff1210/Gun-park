...
const OPENROUTER_API_KEY = process.env.OPENROUTER_API_KEY || process.env.OPENAI_API_KEY || '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';
...
async function generateSentences() {
  try {
    if (!OPENROUTER_API_KEY || OPENROUTER_API_KEY === 'your-api-key-here') {
      console.warn('OpenRouter API key not configured, using default sentences');
...
async function analyzeResponse(originalSentence, userResponse, timeSpent) {
  try {
    if (!OPENROUTER_API_KEY || OPENROUTER_API_KEY === 'your-api-key-here') {
      console.warn('OpenRouter API key not configured, using basic error calculation');
...
async function calculateExpectedTime(sentence, targetWPM) {
  try {
    if (!OPENROUTER_API_KEY || OPENROUTER_API_KEY === 'your-api-key-here') {
      console.warn('OpenRouter API key not configured, using basic time calculation');
      const wordCount = sentence.trim().split(/\s+/).length;
...
Réponds UNIQUEMENT avec le nombre de secondes, sans unité ni explication. Par exemple: "12.5"`
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error(`OpenRouter API error for time calculation: ${response.status} - ${errorText}`);
