...
// HTML to Image API Configuration (Free tier: 50 images/month)
const HTML_TO_IMAGE_API = 'https://htmlcsstoimage.com/demo_run';
const HTML_TO_IMAGE_FALLBACK = 'https://api.screenshotmachine.com';
...
// Profile card generation using HTML-to-Image API
async function generateProfileCard(user) {
  try {
    console.log('Generating profile card for user:', user.username);
    
    // Create HTML template (same as before, just without puppeteer)
...
<body>
    ...
    // Try multiple HTML-to-Image services
    return await generateImageFromHTML(htmlTemplate);
...
// HTML to Image conversion using free APIs
async function generateImageFromHTML(htmlTemplate) {
  console.log('Converting HTML to image...');
  
  // Try multiple free services as fallbacks
  const services = [
    {
      name: 'htmlcsstoimage.com demo',
    ...
    },
    {
      ...
      params: new URLSearchParams({
        access_key: 'demo', // Free demo key - limited usage
        url: `data:text/html;charset=utf-8,${encodeURIComponent(htmlTemplate)}`,
        format: 'png',
        width: '700',
        height: '400',
        fresh: 'true',
  ...
  for (const service of services) {
    try {
      ...
      if (response.ok) {
        ...
        console.log(`Successfully generated image using ${service.name}`);
  ...
  // If all services fail, create a simple text-based fallback image
  console.log('All services failed, creating fallback text image...');
  return createFallbackImage(htmlTemplate);
}

// Fallback: Create a simple text-based image as last resort
function createFallbackImage(htmlTemplate) {
  // Extract user data from HTML template for text-based card
  const usernameMatch = htmlTemplate.match(/<h1 class="name">([^<]+)<\/h1>/);
  const username = usernameMatch ? usernameMatch[1] : 'USER';
  
  // Create a simple SVG as PNG fallback
...
<svg width="700" height="400" xmlns="http://www.w3.org/2000/svg">
  ...
  <rect width="700" height="400" fill="url(#bg)" rx="15"/>
  <text x="350" y="180" font-family="Arial, sans-serif" font-size="32" font-weight="bold" fill="#FFD700" text-anchor="middle">${username}</text>
  <text x="350" y="220" font-family="Arial, sans-serif" font-size="18" fill="#C0C0C0" text-anchor="middle">CARTE DE PROFIL</text>
  <text x="350" y="260" font-family="Arial, sans-serif" font-size="14" fill="#888" text-anchor="middle">Générée par GUN PARK Bot</text>
  <rect x="20" y="20" width="40" height="40" fill="none" stroke="#FFD700" stroke-width="2"/>
  <rect x="640" y="340" width="40" height="40" fill="none" stroke="#FFD700" stroke-width="2"/>
</svg>`;

  // Convert SVG to PNG buffer (this is a simplified approach)
  const svgBuffer = Buffer.from(svg);
  console.log('Created fallback SVG image');
  return svgBuffer;
}
...
class ChronometerService {
  ...
  startActualChronometer() {
    ...
    // Update chronometer every 500ms to avoid Telegram rate limits
  ...
  try {
    await ctx.editMessageText(`⚡ MODE CHALLENGE - ${sentenceCount} phrase${sentenceCount > 1 ? 's' : ''}\n\n🎯 Maintenant choisis ta vitesse cible (WPM) :\n\n💡 Ta meilleure performance : ${user.bestScore} WPM`, 
      Markup.inlineKeyboard([
        ...
        [Markup.button.callback('🏃 40 WPM', 'wpm_40'), Markup.button.callback('⚡ 50 WPM', 'wmp_50')],
...
bot.action('wmp_50', async (ctx) => { await startChallenge(ctx, 50); });
bot.action('wpm_60', async (ctx) => { await startChallenge(ctx, 60); });
bot.action('wpm_70', async (ctx) => { await startChallenge(ctx, 70); });
bot.action('wmp_80', async (ctx) => { await startChallenge(ctx, 80); });
